...
def read_matched_line_detect(fn):
    ...
    """ 
    table = Table.read(fn, format='ascii', names=('ID_in','xin','yin','l_rest','flux_in','zin',
                                  'NR','ID','XS','YS','l','z','dataflux',
                                  'modflux','fluxfrac','sigma','chi2','chi2s',
                                  'chi2w','gammq','gammq_s','eqw', 'cont', 'separation'))
...
def read_simsrc_in(fn):
    ...
    table = Table.read(fn, format='ascii', names=(
                    'ID','xin','yin','l_rest','flux_in','zin'), comment='#')
...
def read_daophot(fn):
    ...
    table : astropy.table.Table
        the rest of the table
    """
    table = Table.read(fn, format='ascii', data_start=2, names=['ID', 'icx', 'icy', 'mag', 'mag_std', 'sky', 'niter', 'CHI', 'SHARP'])
