...
class Shot(object):
    ...
    def __init__(self, shot_dir):
        ...
        Read in the list of dither positions as a function of IHMPID. Read in tracker info 
        and guide probe data for each dither from files is `shot_dir`. Initialize the 
        illumination and transparency servers.  
        ...
        # set up image quality and 
        # illumination model servers
        self.fwhm_servers = []
        self.illu_servers = []  
        self.transparency = []
        ...
        for dither in range(3):
            self.fwhm_servers.append(ImageQualityServer(self.gp_fwhm[dither], 
                                                        self.gp_x[dither], self.gp_y[dither]))

            self.illu_servers.append(IlluminationServer(self.tracker_position[dither]))
          
            # transparency is the normalisation of the GP after correcting for
            # illumination, divide normalisation by illumination at the guide probe
            self.transparency.append(self.gp_normalisation[dither]/self.illu_servers[dither].
                                     illumination(self.gp_x[dither], self.gp_y[dither]))
    ...
    def normalisation(self, x, y, dither):
        ...
        dither : int
            the dither number (0 to 2)
        """  
        return self.illu_servers[dither].illumination(x,y)*self.transparency[dither]
